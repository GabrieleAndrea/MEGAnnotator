...
if [ $TECHNOLOGY = '454' ] ; then
	...
	if [ $READS = 'Single-Read' ] ; then
		...
		cp $PAIRED1 temp/input1_temp.fastq ;
		cp $PAIRED2 temp/input2_temp.fastq ;

		head -n 1 temp/input1_temp.fastq > temp/input_check_1 ;
...
mv results/${PROJECT}_improved.fasta results/${PROJECT}_multifasta.fsa ;
